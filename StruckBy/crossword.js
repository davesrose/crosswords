...
function Crossw1rd(container_id) {
  ...
  this.init = function(id) {
    ...
    this.populateClues(function() {
			...
			var state = sessionStorage.getItem('crossw1rd.'+id);
  ...
  // initializer2- show answers
  this.init2 = function(id) {
    var c = $("#"+container_id);
    c.empty();
    this.id = id; 
    this.container = $('<div class="crossw1rd"></div>').appendTo(c);
    this.populateClues(function() {
			...
			self.drawControls();
			self.adjustDimensions();
			//self.mapKeyBindings2();
			// check for & load saved state
			//var state = sessionStorage.setItem('crossw1rd.'+self.id, self.getState());;
			//if (state!=null) self.resume(state);
			// turn on autosave
			//if (self.autosave) {
				//setInterval(self.save, 1000);
  ...
  // initializer for reset and clear (need to reload in order to reset key binding)
  this.init4 = function(id) {
	  location.reload();
	$('.word').removeClass('word');
    var c = $("#"+container_id);
    c.empty();
  ...
  this.drawGrid = function() {
    ...
    for (var r=0; r<this.height; r++) {
      var row = $('<div class="crossRow"></div>').appendTo(this.grid);
      this.cells[r] = [];
  ...
  this.drawAnswers = function() {
    ...
    // populate letters
      ...
      for (var c=0; c<clue.a.length; c++) {
        ...
        if (cell.find('.letter').length==0) {
			$('<span class="letter"></span>').appendTo(cell);
			$(cell).append(clue.a.slice(c,(c+1)));
        }
      }
    }	  
  }
  ...
  this.drawControls = function() {
	...
	var check = $('<button style="margin-left: 10px;">Check Answers</button>').appendTo(div);
	var clear = $('<button style="margin-left: 10px;">Clear boxes</button>').appendTo(div);
	var show = $('<button style="margin-left: 10px;">Show Answers</button>').appendTo(div);
    reset.click(this.reset);
	check.click(this.check);
	show.click(this.clickAnswers);
  ...
  this.activateCell = function(cell, changedDir) {
    this.grid.find(".crossRow > div").removeClass('active');
    this.grid.find(".crossRow > div").removeClass('word');
    cell = $(cell);
    cell.addClass('active');
    cell.addClass('word');
	cell.width(27).height(27);
    if (this.direction=='A') {
      var prev = cell;
  ...
  this.mapKeyBindings = function() {
    ...
    this.grid.find(".crossRow > div").click(function(ev) {
  ...
  this.save = function() {
    if (!self.saved) {
		sessionStorage.setItem('crossw1rd.'+self.id, self.getState());
      self.saved = true;
    }
  }
  ...
  this.clear = function() {
    if (confirm('Clear your answers: Are you sure?')) {
		sessionStorage.setItem('crossw1rd.'+self.id, '');
      self.init4(self.id);
    }
  }
  
  //show answers
  this.clickAnswers = function() {
	  self.init2('puzzle');
  }
